...
const Header = () => {
  ...
  return (
    <header
      className={clsx(
        "fixed top-0 left-1/2 -translate-x-1/2 z-50 w-full mx-auto flex justify-between items-center transition-all duration-300",
        {
          ...
          "max-w-[1024px] p-6 rounded-3xl mt-6 bg-[#1B1E23] ring-1 ring-[#999999]": isScrolled,
    ...
    >
      ...
      >
        <CodeXml className={clsx("text-primary-blue", { "w-10 h-10": !isScrolled, "w-8 h-8": isScrolled })} strokeWidth={3} />

        <h1 className={clsx("text-white font-bold", { "text-[28px]": !isScrolled, "text-2xl": isScrolled })}>Márcio Jorge</h1>
      </Link>

      <nav className={clsx("flex items-center", { "gap-9": !isScrolled, "gap-6": isScrolled })}>
        {navbarOptions.map((option) => (
          ...
          >
            <h3 className={clsx("text-text-secondary font-medium hover:text-white", { "text-[20px]": !isScrolled, "text-lg": isScrolled })}>
              {option.label}
            </h3>
          </Link>
        ))}
